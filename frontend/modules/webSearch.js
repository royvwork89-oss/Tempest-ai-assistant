...
_enabledProviders = data.enabledProviders ||
  Object.entries(data.config?.providers || {})
    ...
    // Restaurar provider preferido del localStorage
...
function _renderButton() {
  ...
  // Insertar antes del botón enviar
...
function _toggle() {
  ...
  _button.title = _active
    ? `Búsqueda web activa — ${_providerLabel(_provider)}`
    : 'Activar búsqueda web';
}

function _providerLabel(p) {
  const labels = { searxng: 'SearXNG', brave: 'Brave' };
  return labels[p] || p;
}

// Llamado desde chat.js para incluir en el request
export function getWebSearchConfig() {
  if (!_active || !_provider) return {};
  return { webSearch: true, searchProvider: _provider };
}

// Llamado desde settings.js cuando el usuario cambia de provider
export function setProvider(provider) {
  if (!_enabledProviders.includes(provider)) return;
  _provider = provider;
  localStorage.setItem('tempest_search_provider', provider);
  if (_button && _active) {
    _button.title = `Búsqueda web activa — ${_providerLabel(provider)}`;
  }
}